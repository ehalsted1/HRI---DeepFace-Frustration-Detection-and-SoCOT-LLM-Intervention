import json
import logging
import os
import shutil
import time

import numpy as np
import pyttsx3
import sounddevice as sd
from faster_whisper import WhisperModel
from pathlib import Path

import cv2

#ollamama
from ollama import chat 


# from google import genai
import objc
# from google.genai import types
import re
import os
import threading 
from enum import Enum, auto
import re

from queue import Queue

from primitives import Primitives


class LLM_State(Enum):
    IDLE = auto()
    THINKING = auto()
    SPEAKING = auto()
    LISTENING = auto()
    SHUTDOWN = auto()


class InteractionState:
    def __init__(self):
        self.lock = threading.Lock()
        self.llm_state = LLM_State.IDLE

    def set(self, state):
        with self.lock:
            self.llm_state = state

    def get(self):
        with self.lock:
            return self.llm_state

# from Robot_Vision.facial_recognition import FacialRecoginition
from logging_config import init_logging, resolve_debug_flag

# Path for user conversation locations
DB_PATH = os.path.join(os.getcwd(), "database_llm")

def create_fresh_unknown_id(base_dir="database_llm"):
    """
    Always create a brand-new folder unknown_<id>,
    regardless of whether the person was seen before.
    """
    os.makedirs(base_dir, exist_ok=True)

    pattern = re.compile(r"^unknown_(\d+)$")
    ids = []

    for name in os.listdir(base_dir):
        m = pattern.match(name)
        if m:
            ids.append(int(m.group(1)))

    next_id = max(ids, default=0) + 1
    folder_name = f"unknown_{next_id}"
    folder_path = os.path.join(base_dir, folder_name)

    os.makedirs(folder_path, exist_ok=True)

    return folder_path, folder_name

class Conversation(object):

    def __init__(self, mode="normal", conversation_path="", debug=None, interaction_state=None):
        self.debug = init_logging(debug)
        self.stt_result = ""
        self.engine = pyttsx3.init('nsss')
        self.engine.setProperty('rate', 120)
        voices = self.engine.getProperty('voices')
        #self.engine.setProperty('voice', voices[1].id)
        self.interaction_state = interaction_state
        self.mode = mode
        


        folder_path, folder_name = create_fresh_unknown_id()
        conversation_path = os.path.join(folder_path, "conversation.json")

        with open(conversation_path, "w") as f:
            json.dump({"messages": []}, f, indent=2)

        self.conversation_path = conversation_path
        self.conversation_name = folder_name
        self._update_logger_name()
        self.whisper_model = WhisperModel("tiny.en", device="cpu", compute_type="int8")
        
        # Audio calibration state
        self.default_sample_rate = 16000
        self.calibrated_noise_floor = None
        self.calibrated_threshold = None
        self.calibrated_sample_rate = None
        self.calibration_ms = None
        
        # Starting prompts for conversations
        prompt = {"role": "system", "content":  """
                            System Instruction: You are a gentle robot looking to conduct a reminiscence activity with someone to have a meaningful conversation.
                            Start off with a questions about hometown, important people, key events, music, or memories.
                            Then continue with light hearted and friendly questions from there, asking questions to prompt about important memories related to the topic. 
                            You are allowed to be empathetic and curious, but only provide appropiate responses.
                            You are not allowed to ask about politics, religion, or dating avenues. Too personal could be offputting.
                            Make sure to only ask a question in your response within one sentence!!
                            ONE SENTENCE QUESTION RESPONSE ONLY!!
                            Please start us off with a question!
                            """}
        
          # Collect Conversation History
        with open(self.conversation_path, "r") as f:
            data = json.load(f)
        self.conversation = data["messages"]
        self.logger.info(
            "Conversation initialised for %s (path=%s, debug=%s).",
            self.conversation_name,
            self.conversation_path,
            self.debug,
        )

        self.conversation.append(prompt)
        self.remember_conversation() 
        self.conversation_path = conversation_path

        self.model = 'qwen3:1.7b'

        
        # Audio Calibration Setup
        try:
            self.calibrate_noise_floor(duration=1.0, sample_rate=self.default_sample_rate)
        except Exception:
            self.logger.warning("Noise floor calibration failed during init; will attempt later when recording.")

    def _update_logger_name(self):
        suffix = "Unknown"
        self._logger_name = f"{__name__}.Conversation[{suffix}]"

    @property
    def logger(self):
        init_logging(self.debug)
        name = getattr(self, "_logger_name", f"{__name__}.Conversation")
        logger = logging.getLogger(name)
        logger.setLevel(logging.DEBUG if self.debug else logging.INFO)
        return logger
    
    def extract_final_answer(self, text):
        '''getting rid of all the think and extra stuff'''
        cleaned = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL)
        return cleaned.strip()
    
    
    def run_ollama(self, skip=False):

        if skip:
            return "This is a test to see if the TTS is working"
        self.logger.debug("Sending prompt to LLM")

        response = chat(
            model = self.model,
            messages = self.conversation,
            stream=False
        )


        assistant_text = response["message"]["content"]

        cleaned = self.extract_final_answer(assistant_text)

        print(cleaned)

        self.conversation.append({'role': 'assistant', 'content': cleaned})
        self.logger.debug("LLM response: %s", cleaned)

        return cleaned


    def record_and_transcribe(self, duration=2, sample_rate=16000):
        """
        Record audio and transcribe it to text
        """
        # self.logger.info("Recording audio for %.2fs at %sHz.", duration, sample_rate)
        if self.interaction_state:
            self.interaction_state.set(LLM_State.LISTENING)
        audio = sd.rec(int(duration * sample_rate), samplerate=sample_rate, channels=1, dtype='float32')
        sd.wait()
        self.logger.debug("Transcribing recorded audio.")
        segments, _ = self.whisper_model.transcribe(audio.flatten(), beam_size=1)
        text = " ".join(seg.text.strip() for seg in segments)
        self.logger.debug("Transcription result: %s", text)
        return text

    def calibrate_noise_floor(self, duration=1.0, sample_rate=16000):
        """
        Calibrate the noise floor
        """
        self.logger.info("Calibrating noise floor for %ss at %sHz. Please remain silent.", duration, sample_rate)
        chunk_ms = 50
        chunk_size = int(sample_rate * (chunk_ms / 1000.0))
        frames_rms = []
        start_time = time.time()
        with sd.InputStream(samplerate=sample_rate, channels=1, dtype='float32') as stream:
            while (time.time() - start_time) < duration:
                data, _ = stream.read(chunk_size)
                if data is None:
                    continue
                mono = data.reshape(-1)
                rms = float(np.sqrt(np.mean(np.square(mono)) + 1e-12))
                frames_rms.append(rms)
        if len(frames_rms) == 0:
            raise RuntimeError("No audio captured during calibration")
        noise_floor = float(np.median(frames_rms))
        threshold = max(noise_floor * 2.5, 1e-3)
        self.calibrated_noise_floor = noise_floor
        self.calibrated_threshold = threshold
        self.calibrated_sample_rate = sample_rate
        self.calibration_ms = int((time.time() - start_time) * 1000)
        self.logger.info(
            "Noise floor calibrated in %sms: noise_floor=%.6f, threshold=%.6f",
            self.calibration_ms,
            noise_floor,
            threshold,
        )

    def record_and_transcribe_until_silence(
        self,
        min_duration=5.0,
        silence_duration=5.0,
        max_duration=30.0,
        sample_rate=16000,
    ):
        """
        Record audio and transcribe it to text until silence is detected
        """
        # Ensure calibration for this sample rate
        if self.interaction_state:
            self.interaction_state.set(LLM_State.LISTENING)

        if self.calibrated_threshold is None or self.calibrated_sample_rate != sample_rate:
            self.logger.debug("No calibration for sample_rate=%s. Calibrating now...", sample_rate)
            try:
                self.calibrate_noise_floor(duration=0.5, sample_rate=sample_rate)
            except Exception:
                self.logger.warning("Calibration failed; proceeding with conservative default threshold.")
                self.calibrated_noise_floor = 1e-3
                self.calibrated_threshold = 1e-3
                self.calibrated_sample_rate = sample_rate

        self.logger.info(
            "Recording until silence (min_duration=%.2f, silence_duration=%.2f, max_duration=%.2f, sample_rate=%s).",
            min_duration,
            silence_duration,
            max_duration,
            sample_rate,
        )
        chunk_ms = 50
        chunk_size = int(sample_rate * (chunk_ms / 1000.0))
        frames = []
        start_time = time.time()
        last_voice_time = start_time

        with sd.InputStream(samplerate=sample_rate, channels=1, dtype='float32') as stream:
            while True:
                data, _ = stream.read(chunk_size)
                if data is None:
                    continue
                mono = data.reshape(-1)
                frames.append(mono.copy())

                rms = float(np.sqrt(np.mean(np.square(mono)) + 1e-12))

                elapsed = time.time() - start_time
                threshold = self.calibrated_threshold if self.calibrated_threshold is not None else 1e-3

                if rms > threshold:
                    last_voice_time = time.time()

                # Stop if recorded long enough and observed sustained silence
                if elapsed >= min_duration and (time.time() - last_voice_time) >= silence_duration:
                    break

                # Hard stop at max_duration
                if elapsed >= max_duration:
                    break

        # Concatenate and measure actual recorded duration
        if len(frames) == 0:
            self.logger.warning("No audio captured during silence-detection recording.")
            return ""

        audio = np.concatenate(frames)
        recorded_seconds = len(audio) / float(sample_rate)
        self.logger.info("Recorded duration: %.2fs", recorded_seconds)

        # Transcribe
        self.logger.debug("Transcribing captured audio.")
        t0 = time.time()
        segments, _ = self.whisper_model.transcribe(audio, beam_size=1)
        text = " ".join(seg.text.strip() for seg in segments)
        transcribe_seconds = time.time() - t0
        self.logger.debug("Transcription completed in %.2fs. Result: %s", transcribe_seconds, text)
        return text
    
    def test_affect_capture_window(self):
        primitive = Primitives()
        summary = []
        print("starting recording ")
        summary.append(primitive.capture_affect_window(duration_s=3.0))
        time.sleep(3)
        summary.append(primitive.capture_affect_window(duration_s=1.5))

        print(summary)
    
    def record_mp4_and_transcribe_until_silence(
        self,
        min_duration=5.0,
        silence_duration=5.0,
        max_duration=30.0,
        sample_rate=16000,
        folder_name="recordings",
        video_device=None,
        audio_device=None
    ):
        """
        Record audio and transcribe it to text until silence is detected

        """
        # Ensure calibration for this sample rate
        if self.interaction_state:
            self.interaction_state.set(LLM_State.LISTENING)
        
        #setting up output files
        ts = time.strftime("%Y%m%d_%H%M%S")
        outdir = Path(self.conversation_path) / folder_name / ts
        outdir.mkdir(parents=True, exist_ok=True)

        mp4_path = outdir / "recording.mp4"
        wav_path = outdir / "audio.wav"
        txt_path = outdir / "transcript.txt"
        meta_path = outdir / "meta.json"

        #recording mp4 with ffmpeg
        is_linux_video = video_device and str(video_device).startswith("/dev/")

        if is_linux_video:
            if audio_device is None: 
                audio_device = "default"
            # ffmpeg_cmd = [
                
            # ]

        if self.calibrated_threshold is None or self.calibrated_sample_rate != sample_rate:
            self.logger.debug("No calibration for sample_rate=%s. Calibrating now...", sample_rate)
            try:
                self.calibrate_noise_floor(duration=0.5, sample_rate=sample_rate)
            except Exception:
                self.logger.warning("Calibration failed; proceeding with conservative default threshold.")
                self.calibrated_noise_floor = 1e-3
                self.calibrated_threshold = 1e-3
                self.calibrated_sample_rate = sample_rate

        self.logger.info(
            "Recording until silence (min_duration=%.2f, silence_duration=%.2f, max_duration=%.2f, sample_rate=%s).",
            min_duration,
            silence_duration,
            max_duration,
            sample_rate,
        )
        chunk_ms = 50
        chunk_size = int(sample_rate * (chunk_ms / 1000.0))
        frames = []
        start_time = time.time()
        last_voice_time = start_time

        with sd.InputStream(samplerate=sample_rate, channels=1, dtype='float32') as stream:
            while True:
                data, _ = stream.read(chunk_size)
                if data is None:
                    continue
                mono = data.reshape(-1)
                frames.append(mono.copy())

                rms = float(np.sqrt(np.mean(np.square(mono)) + 1e-12))

                elapsed = time.time() - start_time
                threshold = self.calibrated_threshold if self.calibrated_threshold is not None else 1e-3

                if rms > threshold:
                    last_voice_time = time.time()

                # Stop if recorded long enough and observed sustained silence
                if elapsed >= min_duration and (time.time() - last_voice_time) >= silence_duration:
                   #capture 
                   end_window_time = 2.0
                   #sample and enuquee frames 
                   #run aggregation and return fetures along with transcript 
                   break

                # Hard stop at max_duration
                if elapsed >= max_duration:
                    break

        # Concatenate and measure actual recorded duration
        if len(frames) == 0:
            self.logger.warning("No audio captured during silence-detection recording.")
            return ""

        audio = np.concatenate(frames)
        recorded_seconds = len(audio) / float(sample_rate)
        self.logger.info("Recorded duration: %.2fs", recorded_seconds)

        # Transcribe
        self.logger.debug("Transcribing captured audio.")
        t0 = time.time()
        segments, _ = self.whisper_model.transcribe(audio, beam_size=1)
        text = " ".join(seg.text.strip() for seg in segments)
        transcribe_seconds = time.time() - t0
        self.logger.debug("Transcription completed in %.2fs. Result: %s", transcribe_seconds, text)
        return text

    def text_to_speech(self, response):
        """
        Text to speech the response
        """
        if self.interaction_state:
            self.interaction_state.set(LLM_State.SPEAKING)

        self.engine.say(response)
        self.engine.runAndWait()

        if self.interaction_state:
            self.interaction_state.set(LLM_State.IDLE)
    
    def remember_conversation(self):
        """
        End the conversation and save the conversation history
        """
        with open(self.conversation_path, "w") as f:
            json.dump({"messages": self.conversation}, f, indent=2)
        self.logger.info("Conversation history written to %s.", self.conversation_path)

    def llm_worker(self,llm_output_queue,stop_event):
        print("Starting conversation")
        init_logging(self.debug)

        response = self.run_ollama()
        self.logger.info("[LLM]: %s", response)

        llm_output_queue.put(response)
        self.text_to_speech(response)

        self.logger.info("Conversation loop starting for %s (debug=%s).",
                        self.conversation_name, self.debug)

        while not stop_event.is_set():

            self.stt_result = self.record_and_transcribe_until_silence()

            if self.stt_result == "" or self.stt_result == " ":
                self.logger.warning("No response detected from user during conversation.")
                self.engine.say("Sorry I couldn't catch that, could you please say that again?")
                self.engine.runAndWait()
                continue
            if "done" in self.stt_result.lower() or "quit" in self.stt_result.lower() or "end conv" in self.stt_result.lower() or "and conv" in self.stt_result.lower():
                self.logger.info("User requested to end the conversation.")
                stop_event.set()
                break

            self.conversation.append({
                'role': 'user',
                'content': self.stt_result
            })

            self.logger.info("[User]: %s", self.stt_result)

            responding = self.run_ollama()
            self.logger.info("[LLM]: %s", responding)

            # 
            llm_output_queue.put(responding)

            self.text_to_speech(responding)
            self.logger.info("[LLM]: Responded TTS")

            self.remember_conversation()

    def format_multiple_options(self, skip=False):
        '''
        formats the various optiosn and should be used 
        to determine the responding variable within the 
        current main format
        '''
        with open(self.conversation_path, "r") as f:
            data = json.load(f)
        
        messages = data["messages"]
        target_label = "model"

        # last_index = next(i for i in reversed(range(len(data))) if data[i].get("role") == target_label)

        # last_model_msg = messages[last_index]
        # parts = last_model_msg.get("parts", [])
        # last_model_txt = parts[0]["text"] if parts else None

        # last_model_message = next(msg for msg in reversed(data["messages"]) if msg.get("role") == "model")
        # last_model_text = " ".join(p["text"] for p in last_model_message.get("parts", []))
    
        #frustrate (3 options, repeat same question, I forgot what we were talking about, can you repeat that )
        #check in type (3 options, neutral positive, negative)
    
        #number of responses + types
        responses = []

        regular_response = self.run_ollama()

        forgetful_response = "I forgot what we were talking about. Can you remind me?"

        # repeated_response = last_model_text

        neutral_checkin = "Should we keep going, or would you like to change subjects?"

        negative_checkin = "it seems like this brings up some heavy feelings. We can change the subject if you want."

        positive_checkin = "it seems like you are enjoying talking. I'm happy to keep listening."

        quit_option = "It was great talking to you, I'll be going to sleep now."
    
        responses.extend([regular_response, forgetful_response, neutral_checkin, positive_checkin, negative_checkin, quit_option])
        responses_menu = ["Regular Response", "Forgetful Response", "Neutral Check In", "Positive Check In", "Negative Check In", "Quit"]
        self.logger.info("Generating response choices...")
        num_choices = len(responses)

        for i in range (num_choices):
            self.logger.info(f"{i}): {responses_menu[i]}")
            
        text = input("Enter the number to respond with: ")
        self.logger.info(f"Selected response: {responses[int(text)]}")
        response = responses[int(text)]

        return response

            
    def main(self):
        print("Starting conversation")
        init_logging(self.debug)
        response = self.run_ollama()
        self.logger.info("[LLM]: %s", response)
        self.text_to_speech(response)
        quit_option = "It was great talking to you, I'll be going to sleep now."
        
        print("recording")
        self.logger.info("Conversation loop starting for %s (debug=%s).", self.conversation_name, self.debug)
        while True:
            self.stt_result = self.record_and_transcribe_until_silence()
            if self.stt_result == "" or self.stt_result == " ":
                self.logger.warning("No response detected from user during conversation.")
                self.engine.say("Sorry I couldn't catch that, could you please say that again?")
                self.engine.runAndWait()
                continue
            if "done" in self.stt_result.lower() or "quit" in self.stt_result.lower() or "end conv" in self.stt_result.lower() or "and conv" in self.stt_result.lower():
                self.logger.info("User requested to end the conversation.")
                break
            self.conversation.append({'role': 'user', 'content': self.stt_result})
            self.logger.info("[User]: %s", self.stt_result)
            
            responding = self.run_ollama()
            self.logger.info("[LLM]: %s", responding)

            self.text_to_speech(responding)
            self.logger.info("[LLM]: Responded TTS")

            self.remember_conversation()
            
            if responding == quit_option:
                self.logger.info("User requested to end the conversation.")
                if self.interaction_state:
                    self.interaction_state.set(LLM_State.SHUTDOWN)
                break

        if self.interaction_state:
            self.interaction_state.set(LLM_State.SHUTDOWN)
        self.logger.info("Conversation loop concluded for %s.", self.conversation_name)
    
if __name__ == "__main__":
    conversation_path = os.path.join(DB_PATH)
    conversation = Conversation(conversation_path=conversation_path)
    # conversation.main()
    conversation.test_affect_capture_window()
