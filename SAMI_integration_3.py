'''
This is the integration based on the working 
SAMI_integration_2.py file, but now implenting 
the llm code that implements choice ehehe
'''
    ...
    try:
        ...
        while True:
            ...
            if "done" in convo.stt_result.lower() or "quit" in convo.stt_result.lower() or "end conv" in convo.stt_result.lower() or "and conv" in convo.stt_result.lower():
                ...
                break
            convo.conversation.append({'role': 'user', 'parts': [{"text": convo.stt_result}]})
            convo.logger.info("[User]: %s", convo.stt_result)
            
            responding = convo.format_multiple_options()
