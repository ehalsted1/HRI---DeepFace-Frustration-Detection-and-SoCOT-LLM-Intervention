...
class Conversation(object):
    ...
    def __init__(self, conversation_path="", debug=None, interaction_state=None):
        ...
        #SAMI 
        self.robot = SAMIControl(arduino_port=ARDUINO_PORT, audio_folder="audio", starting_voice="Matt")
    ...
    def llm_chat(self, robot, skip=False):
        """
        Chat with the LLM and return the response
        """
        #setting flags
        if self.interaction_state:
            self.interaction_state.set(LLM_State.THINKING)
        ...
        robot.run_behavior_block("empatheticQuestion.json")
    ...
    def record_and_transcribe_until_silence(
        ...
        robot.run_behavior_block("nodding.json")
    ...
    def main(self):
        print("Starting conversation")
        init_logging(self.debug)
        response = self.llm_chat(self.robot)
        self.logger.info("[LLM]: %s", response)
        self.text_to_speech(response)

        self.robot.run_behavior_block("Home.json")
        self.robot.run_behavior_block("Wave.json")
        self.robot.run_behavior_block("Home.json")
        
        print("recording")
        self.logger.info("Conversation loop starting for %s (debug=%s).", self.conversation_name, self.debug)
        while True:
            self.stt_result = self.record_and_transcribe_until_silence(self.robot)
            if self.stt_result == "" or self.stt_result == " ":
                self.logger.warning("No response detected from user during conversation.")
                self.engine.say("Sorry I couldn't catch that, could you please say that again?")
                self.engine.runAndWait()
            ...
            responding = self.llm_chat(self.robot)
            self.logger.info("[LLM]: %s", responding)
